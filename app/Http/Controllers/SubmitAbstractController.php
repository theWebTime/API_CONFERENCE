...
class SubmitAbstractController extends BaseController
{
    ...
    {
        try {
            $data = SubmitAbstract::join('conferences', 'conferences.id', '=', 'submit_abstracts.conferences_id')->select('submit_abstracts.id', 'submit_abstracts.conferences_id', 'name', 'submit_abstracts.title', 'submit_abstracts.email', 'alternative_email', 'phone_number', 'whatsapp_number', 'city', 'organization', 'submit_abstracts.country_id', 'interested_in', 'abstract_title', 'message', 'submit_abstract_file')->where(function ($query) use ($request) {
